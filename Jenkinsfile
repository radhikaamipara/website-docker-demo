pipeline {  
    ...
    environment {  
        ...
        AWS_ACCOUNT_ID = '765835831390'  
        IMAGE_TAG = "${env.BUILD_NUMBER}"  
        IMAGE_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:${IMAGE_TAG}"  
        LATEST_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:latest"  
        DEPLOY_SERVER = '65.0.182.7'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'main', url: 'https://github.com/radhikaamipara/website-docker-demo.git'  
            }  
        }  
    ...
    steps {
        sh '''
        aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 765835831390.dkr.ecr.ap-south-1.amazonaws.com
        '''
    }
}
